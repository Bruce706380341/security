...
SECURITY_VERSION=("$ES_PLUGINS_DIR/opendistro-security/opendistro-security-*.jar")
SECURITY_VERSION=$(echo $SECURITY_VERSION | sed 's/.*opendistro-security-\(.*\)\.jar/\1/')
...
$SUDO_CMD chmod +x "$ES_PLUGINS_DIR/opendistro-security/tools/securityadmin.sh"
...
echo $SUDO_CMD \""$ES_PLUGINS_DIR/opendistro-security/tools/securityadmin.sh"\" -cd \""$ES_PLUGINS_DIR/opendistro-security/securityconfig"\" -icl -key \""$ES_CONF_DIR/kirk-key.pem"\" -cert \""$ES_CONF_DIR/kirk.pem"\" -cacert \""$ES_CONF_DIR/root-ca.pem"\" -nhnv | $SUDO_CMD tee -a securityadmin_demo.sh > /dev/null
